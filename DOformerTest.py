...
from model_function.model_function import DOformer, evaluate, compute_ig_feature_importance
...
print(f"Save test predictions to {output_path}")

# Integrated Gradients (IG)
n_steps = 20  # number of steps for IG approximation
# feature names
feature_names = numeric_cols + categorical_cols + ['Date_sin', 'Date_cos', 'Lat_sin', 'Lat_cos', 'Lon_sin', 'Lon_cos']
# dataset for IG computation
full_dataset = ImprovedTimeSeriesDataset(
...
global_importance, global_delta = compute_ig_feature_importance(model, train_loader, device, n_steps, feature_names)
...
global_delta_df = pd.DataFrame({'Global_Delta': [global_delta]})
global_delta_df.to_csv('global_delta.csv', index=False)
